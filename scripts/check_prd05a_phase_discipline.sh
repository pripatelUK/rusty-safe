...
status="PASS"
reason="phase branch and commit discipline checks passed"

if [[ "$branch" != "main" && "$branch" != "sign-tx" ]]; then
  if [[ ! "$branch" =~ ^feat/prd05a-e2e-e[0-9]+-[a-z0-9-]+$ ]]; then
    status="FAIL"
    reason="branch name does not match feat/prd05a-e2e-e<phase>-<slug>: ${branch}"
  fi
fi

commit_sample="$(git log -n 30 --pretty=%s 2>/dev/null || true)"
if [[ "$status" == "PASS" ]]; then
  if ! echo "$commit_sample" | rg -q "E[0-9]-T[0-9]|gate-green"; then
    status="FAIL"
    reason="recent commit messages do not include E*-T* task IDs or gate-green markers"
  fi
fi
